export default (name: string, ignore: string[]):boolean => {
    for(let i = 0; i < ignore.length; i++){
        let toIgnore: string = ignore[i];
        ...
        if(toIgnore.substring(0, 1) === '/'){ // start with "/" char
            if(toIgnore.substring(toIgnore.length-1) === '/'){ // end with "/" char
                ...
                if(toIgnore === name) // not ignore dir
                    return false;
                else if (toIgnore === name.substr(0, toIgnore.length)){ // ignore file which start with same dir
                    return true;
                }
            }
            if(name === toIgnore)
